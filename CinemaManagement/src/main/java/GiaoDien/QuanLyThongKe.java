...
public class QuanLyThongKe extends javax.swing.JFrame {
    ...
    private void LoadTableThongKe() {
        ...
        try {
            PreparedStatement pstmt = con.prepareStatement("SELECT `account`.`name`, `opera`.`opera_name`, `invoice`.`total_price`, `invoice`.`invoice_date` FROM `invoice`, `opera`,`account` WHERE `invoice`.`opera_id`=`opera`.`ID` AND `invoice`.`account_ID`=`account`.`ID` AND `invoice`.`status`=1");
